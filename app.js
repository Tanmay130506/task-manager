...
async function loadTasks() {
    ...
    const response = await fetch(
        `/tasks/my?limit=${appState.limit}&offset=${appState.offset}`,
        { credentials: "include" }
    );
...
async function acceptTask(taskId, button) {
    ...
    // UPDATED URL
    const response = await fetch(`/tasks/accept/${taskId}`, {
        method: "POST",
        credentials: "include"
    });
...
async function reviewTask(taskId, button) {
    ...
    // UPDATED URL
    const response = await fetch(
        `/tasks/review/${taskId}?review=${encodeURIComponent(reviewText)}`,
        {
            method: "POST",
            credentials: "include"
        }
